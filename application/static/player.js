...
document.addEventListener(
  ...
  () => {
    ...
    commentButton.addEventListener("click", () => {
      ...
      makeRequest(jsonData, "POST", "/addComment").catch((err) =>
        ...
        promise
          .then((res) => {
            ...
            document.getElementById(
              "film-rating"
            ).innerHTML = `${res.rating} (${res.rating_count} оценок)`;
          })
    ...
    comments
      .then((res) => {
        ...
        for (let i = 0; i < res.length; i++) {
          makeRequest(`{"id":${res[i].userid}}`, "POST", "/getUsername").then(
            (username) => {
